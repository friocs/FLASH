      ...
      <div class="box">
        <div class="right">
          <div class="nav-box">
            ...
            <span>{{ nftdetails.name ? nftdetails.name + "(" + nftdetails.nftContractName + ")" :
            nftdetails.nftContractName
            }}</span> <img src="../assets/img/authentication.png" alt="" v-if="nftdetails.isAuth == 2" class="nav-img">
          </div>

          <!-- <div class="top">
            <p>{{ nftdetails.nftContractName }}</p>
            <br>
            <p>#{{ nftdetails.nftTokenId }}</p>
          </div> -->
          <div class="top">
            {{ nftdetails.nftContractName }}
            <p>#{{ nftdetails.nftTokenId }}</p>
          ...
          <div class="buy-box" v-if="nftdetails.poolAddr">
            ...
            <p><img src="../assets/img/chain_2.png" alt="">{{ proxy.$filters.numFilters4(nftdetails.nftPrice) }}</p>
            <p>{{ $t('Home.floorPrice') }}(BNB)：<img src="../assets/img/chain_2.png" alt=""><span>{{
            proxy.$filters.numFilters4(poolDeatil.floorPrice) || '--'
            }}</span></p>
            <!-- <div class="buy-btn" @click="sell"><img src="../assets/img/buyPc1.png" alt="">{{ getName }}</div> -->
            <div>
              <el-button type="primary" loading v-if="loading" class="buy-btn">{{ $t('overall.Purchasing') }}
              </el-button>
              <div v-else class="buy-btn" @click="sell"><img src="../assets/img/buyPc1.png" alt="">{{ getName }}</div>
            </div>
          </div>
          <div v-else style="height:.9896rem;display: flex;align-items: center;">
            <el-button type="primary" loading v-if="loading" class="buy-btn sell-btn">{{ $t('overall.Purchasing') }}
            </el-button>
            <div v-else class="buy-btn sell-btn" @click="sell"><img src="../assets/img/buyPc1.png" alt="">{{ getName }}
            </div>
          </div>
          ...
          <div class="table-box">
            ...
            <div class="table-list" v-if="nftdetails.poolAddr">
              ...
              <div class="table-right" style="color:#3B82F6" @click="open1(nftdetails.poolAddr)">
                <span @click.stop="open(nftdetails.contractAddr)" class="go-pool"> {{ $t('NFTdetails.goPool') }}<img
                    src="../assets/img/forward-icon.png" alt=""> </span>
                {{
      ...
      <div class="bottom">
        ...
        <el-table :data="nftdetails.partData" style="width: 100%">
          ...
          <el-table-column prop="price" :label="$t('my.price')" align="center">
            <template #default="scope">
              <div class="table-price"><img src="../assets/img/chain_2.png" alt=""> {{
              proxy.$filters.numFilters4(scope.row.price)
              }}
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('my.Sender')" align="center">
            <template #default="scope">
              <div style="color:#3B82F6;cursor: pointer;"
                @click="open1(scope.row.type == 1 ? scope.row.from : scope.row.to)">{{ scope.row.type == 1 ?
          ...
          <el-table-column :label="$t('my.Receiver')" align="center">
            <template #default="scope">
              <div style="color:#3B82F6;cursor: pointer;"
                @click="open1(scope.row.type == 1 ? scope.row.to : scope.row.from)">{{ scope.row.type == 1 ?
          ...
          <el-table-column :label="$t('my.date')" align="center">
            <template #default="scope">
              <div class="table-time" style="color:#3B82F6;cursor: pointer;" @click="open2(scope.row.ts)">{{ setTime(new
              Date().getTime(), new Date(scope.row.time).getTime())
              }} {{
                $t('browse.ago')
        ...
        <!-- <div class="more" @click="onAddItem"
          v-if="nftdetails.partData.length != nftdetails.transactionHistoryVOList.length">加载更多</div> -->
...
import { switchChainNoArgs, switchChain } from '@/utils/DialogSwitchChain'
...
let poolDeatil = ref({ nftPrice: "", floorPrice: "" })
