  ...
  <el-container>
    <el-menu default-active="2" class="el-menu-vertical-demo" :collapse="isCollapse" @open="handleOpen"
      ...
      <div class="menu-box" v-if="foldShow">
        ...
        <div class="section">
          <div class="right">
            <input type="number" :placeholder="$t('browse.minimum_price')" @keyup="search" v-model="Start" />
    ...
    <el-container>
      <el-main style="padding-top:0px ;">
        ...
        <div style="display:flex; justify-content: flex-end; margin-right:57px;">
          ...
          <div @click="click = 1" class="img_box" :class="click == 1 ? 'click' : ''" style="margin-left: 23px;">
            <img src="@/assets/img/enlarge.png" alt="">
          </div>
          <div class="img_box" :class="click == 1 ? '' : 'click'" @click="click = 2"><img src="@/assets/img/narrow.png"
              alt=""></div>
        </div>
          ...
          <div class="main">
            ...
            <div :class="click == 1 ? 'box' : 'small-box'" v-for="(item, index) in marketList" :key="index">
              ...
              <div class="bottom">
                ...
                <div class="Price">
                  <img src="@/assets/img/BNB.png" alt="" style="margin-right:5px;width:12px;height:12px;">
                  {{ item.price.substring(0, item.price.indexOf(".") + 5) }}
                </div>
              </div>
              <div style="background: #EBEEF2;border-radius: 0px 0px 8px 8px;">
                <div class="btn" @click="buyNow(item.nftId, '1')">
                  <img style=" width: 9px;height: 18px;vertical-align: middle;" src="@/assets/img/buyPc.png" alt="">
                  {{ t('browse.Buy') }}
                </div>
              </div>
            </div>
...
import {
  ...
  computed,
  nextTick,
  PropType, onUpdated, onBeforeUpdate, onActivated
} from "vue";
...
const getMarketList = (loadingInstance?: any) => {
  ...
  if (!seachData.param.contractAddr && nftlist.type == 1) {
    seachData.param.contractAddr = localStorage.getItem('chooseNFTAddress')
  }
  getNftListByMarket(seachData)
    .then((res: any) => {
      console.log(res);
      loading.value = true
      if (seachData.current == 1) {
        marketList.value = [];
      }
      res.rows.forEach(val => {
        marketList.value.push(val)
      });
      loading.value = false;
    ...
    localStorage.setItem('chooseNFTAddress', seachData.param.contractAddr)
...
onBeforeUpdate(() => {
  ...
  console.log("组件更新 ===> onBeforeUpdate")
  if (viewLoad.value == false) {
    seachData.current = 1;
    marketList.value = [];
    getNftListByMarket(seachData)
      .then((res: any) => {
